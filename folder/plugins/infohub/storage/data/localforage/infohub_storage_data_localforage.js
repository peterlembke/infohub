/**
 * infohub_storage_data_localforage
 * Uses the local storage engine indexedDb through the library localForage to store key value
 *
 * @package     Infohub
 * @subpackage  infohub_storage_data_localforage
 * @since       
 * @author      Peter Lembke <info@infohub.se>
 * @license     GPL-3.0-or-later
 * @copyright   Copyright (C) 2010- Peter Lembke
 * @see         https://github.com/peterlembke/infohub/blob/main/folder/plugins/infohub/storage/data/localforage/infohub_storage_data_localforage.md Documentation
 * @link        https://infohub.se/ InfoHub main page
 */
function infohub_storage_data_localforage() {
...
// include "infohub_base.js"
    ...
    const read = function($in = {}) {
        ...
        if (!window.localforage) {
            $in.callback_function({
                'answer': 'false',
                'message': 'localForage is not installed',
            });
        }

        localforage.getItem($in.path).then(function(value) {
            $postExist = 'true';
    ...
    const write = function($in = {}) {
        ...
        localforage.setItem($in.path, $dataString).then(function() {
...
(function(f) {
    if (typeof exports === 'object' && typeof module !==
        'undefined') {module.exports = f();} else if (typeof define ===
        'function' && define.amd) {define([], f);} else {
        var g;
        if (typeof window !==
            'undefined') {g = window;} else if (typeof global !==
            'undefined') {g = global;} else if (typeof self !==
            'undefined') {g = self;} else {g = this;}
        g.localforage = f();
    }
})(function() {
    ...
    return (function e(t, n, r) {
        ...
        1: [
            function(_dereq_, module, exports) {
                (function(global) {
                    'use strict';
                    var Mutation = global.MutationObserver ||
...
//named nextTick for less confusing stack traces
            ...
            function(_dereq_, module, exports) {
                ...
                Promise.prototype.then = function(onFulfilled, onRejected) {
                    if (typeof onFulfilled !== 'function' && this.state ===
                        FULFILLED ||
                        typeof onRejected !== 'function' && this.state ===
                        REJECTED) {
                        return this;
                    }
                    var promise = new this.constructor(INTERNAL);
                    if (this.state !== PENDING) {
                        var resolver = this.state === FULFILLED
                            ? onFulfilled
                            : onRejected;
                ...
                function unwrap(promise, func, value) {
                    immediate(function() {
                        ...
                        if (returnValue === promise) {
                            handlers.reject(promise, new TypeError(
                ...
                function all(iterable) {
                    var self = this;
                    if (Object.prototype.toString.call(iterable) !==
                        '[object Array]') {
            ...
            function(_dereq_, module, exports) {
                ...
                function isIndexedDBValid() {
                    try {
                        ...
                        return (!isSafari || hasFetch) && typeof indexedDB !==
                            'undefined' &&
...
//
                function _checkBlobSupportWithoutCaching(idb) {
                    return new Promise$1(function(resolve) {
                        var txn = idb.transaction(DETECT_BLOB_SUPPORT_STORE,
                            READ_WRITE);
                        var blob = createBlob(['']);
                        txn.objectStore(DETECT_BLOB_SUPPORT_STORE).
                            put(blob, 'key');
                        ...
                        txn.oncomplete = function() {
                            var matchedChrome = navigator.userAgent.match(
                                /Chrome\/(\d+)/);
                            var matchedEdge = navigator.userAgent.match(
                                /Edge\//);
                ...
                function _checkBlobSupport(idb) {
                    ...
                    return _checkBlobSupportWithoutCaching(idb).
                        then(function(value) {
                            supportsBlobs = value;
                ...
                function _deferReadiness(dbInfo) {
                    ...
                    deferredOperation.promise = new Promise$1(
...
// so we have to do it with callbacks
                function createTransaction(dbInfo, mode, callback, retries) {
                    ...
                    try {
                        ...
                        if (retries > 0 &&
                            (!dbInfo.db || err.name === 'InvalidStateError' ||
                                err.name === 'NotFoundError')) {
                            return Promise$1.resolve().then(function() {
                                if (!dbInfo.db || err.name ===
                                    'NotFoundError' &&
                                    !dbInfo.db.objectStoreNames.contains(
                                        dbInfo.storeName) && dbInfo.version <=
                                    dbInfo.db.version) {
                                ...
                                return _tryReconnect(dbInfo).then(function() {
                                    createTransaction(dbInfo, mode, callback,
                                        retries - 1);
                                });
                            })['catch'](callback);
                        }
                ...
                function getItem(key, callback) {
                    ...
                    var promise = new Promise$1(function(resolve, reject) {
                        self.ready().then(function() {
                            createTransaction(self._dbInfo, READ_ONLY,
...
// Iterate over all items stored in database.
                ...
                function setItem(key, value, callback) {
                    ...
                    var promise = new Promise$1(function(resolve, reject) {
                        ...
                        self.ready().then(function() {
                            ...
                            createTransaction(self._dbInfo, READ_WRITE,
                                function(err, transaction) {
                                    if (err) {
                                        return reject(err);
                                    }
                ...
                function dropInstance(options, callback) {
                    ...
                    if (!options.name) {
                        ...
                        var isCurrentDb = options.name === currentConfig.name &&
                            self._dbInfo.db;

                        var dbPromise = isCurrentDb ? Promise$1.resolve(
                            self._dbInfo.db) : _getOriginalConnection(options).
                            then(function(db) {
                        ...
                        if (!options.storeName) {
                            promise = dbPromise.then(function(db) {
                                ...
                                var dropDBPromise = new Promise$1(
                                    function(resolve, reject) {
                            ...
                            promise = dbPromise.then(function(db) {
                                ...
                                var dropObjectPromise = new Promise$1(
                                    function(resolve, reject) {
                                        var req = idb.open(options.name,
                                            newVersion);
...
// storage library.
                function bufferToString(buffer) {
                    ...
                    for (i = 0; i < bytes.length; i += 3) {
                        ...
                        bytes[i + 1] >> 4];
                        base64String += BASE_CHARS[(bytes[i + 1] & 15) << 2 |
                        bytes[i + 2] >> 6];
                        base64String += BASE_CHARS[bytes[i + 2] & 63];
                    }

                    if (bytes.length % 3 === 2) {
                        base64String = base64String.substring(0,
...
// we store binary data with localStorage.
                function serialize(value, callback) {
                    ...
                    if (value &&
                        (valueType === '[object ArrayBuffer]' || value.buffer &&
...
// some kind of arraybuffer/binary data/typed array out of the string.
                function deserialize(value) {
                    ...
                    // done with it.
                    if (value.substring(0, SERIALIZED_MARKER_LENGTH) !==
                        SERIALIZED_MARKER) {
...
// previously exist), using any options set in the config.
                function _initStorage$1(options) {
                    ...
                    var dbInfoPromise = new Promise$1(
                        function(resolve, reject) {
                            // Open the database; the openDatabase API will automatically
                            // create it for us if it doesn't exist.
                            try {
                                dbInfo.db = openDatabase(dbInfo.name,
                                    String(dbInfo.version), dbInfo.description,
                                    dbInfo.size);
                            } catch (e) {
                                return reject(e);
                            }
                ...
                function tryExecuteSql(
                    t, dbInfo, sqlStatement, args, callback, errorCallback) {
                    t.executeSql(sqlStatement, args, callback,
                        function(t, error) {
                ...
                function _setItem(key, value, callback, retriesLeft) {
                    ...
                    var promise = new Promise$1(function(resolve, reject) {
                        self.ready().then(function() {
                            ...
                            var dbInfo = self._dbInfo;
                            dbInfo.serializer.serialize(value,
                        ...
                        promise = new Promise$1(function(resolve) {
                            ...
                            return new Promise$1(function(resolve, reject) {
                                operationInfo.db.transaction(function(t) {
                                    ...
                                    var operations = [];
                                    for (var i = 0, len = operationInfo.storeNames.length; i <
                                    len; i++) {
                                        operations.push(dropTable(
...
// is 0 and there wasn't any saved items yet.
                function _isLocalStorageUsable() {
                    return !checkIfLocalStorageThrows() || localStorage.length >
                        0;
                }
...
// Config the localStorage backend, using options set in the config.
                function _initStorage$2(options) {
                    ...
                    dbInfo.keyPrefix = _getKeyPrefix(options,
                        self._defaultConfig);
...
// They are shared across all instances of localForage.
                ...
                var LibraryMethods = [
                    ...
                    'length',
                    'removeItem',
                    'setItem'].concat(OptionalDriverMethods);
                ...
                var LocalForage = function() {
                    ...
                    LocalForage.prototype.defineDriver = function defineDriver(
                        driverObject, callback, errorCallback) {
                        var promise = new Promise$1(function(resolve, reject) {
                            try {
                                ...
                                var driverMethods = LibraryMethods.concat(
                                    '_initStorage');
                                for (var i = 0, len = driverMethods.length; i <
                                len; i++) {
                                ...
                                var configureMissingMethods = function configureMissingMethods() {
                                    ...
                                    for (var _i = 0, _len = OptionalDriverMethods.length; _i <
                                    _len; _i++) {
                    ...
                    LocalForage.prototype.getDriver = function getDriver(
                        driverName, callback, errorCallback) {
                        ...
                        executeTwoCallbacks(getDriverPromise, callback,
                            errorCallback);
                    ...
                    LocalForage.prototype.setDriver = function setDriver(
                        drivers, callback, errorCallback) {
                        var self = this;
                                ...
                                function driverPromiseLoop() {
                                    while (currentDriverIndex < supportedDrivers.length) {
                                        ...
                                        return self.getDriver(driverName).
                                            then(extendSelfWithDriver)['catch'](
                                            driverPromiseLoop);
                                    }
                        ...
                        executeTwoCallbacks(this._driverSet, callback,
                            errorCallback);
                    ...
                    LocalForage.prototype._wrapLibraryMethodsWithReady = function _wrapLibraryMethodsWithReady() {
                        ...
                        for (var i = 0, len = LibraryMethods.length; i <
                        len; i++) {
...
//# sourceURL=infohub_storage_data_localforage.js
